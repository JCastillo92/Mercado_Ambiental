...
public class Cls_Trueque {
	...
	public String lista_trueques_vendidos_historial(){
		String sql="select tb_historico.descripcion,tb_historico.cantidad,tb_monedas.descripcion,tb_categorias.descripcion,tb_historico.fecha,tb_usuarios.nombre,tb_historico.tipo  "
	...
	public String lista_trueques_vendidos_puja(){
		...
		try{
			...
			while(rs.next()){
				acum_jsp=acum_jsp+"<tr><td>"+rs.getString(1)+"</td><td>"+rs.getInt(2)+"</td><td>"+rs.getString(3)+"</td><td>"+rs.getString(4)+"</td><td>"+rs.getString(5)+"</td><td>"+rs.getString(6)+"</td>"
						+ "<td><span class=\"badge\">Puja</span></td></tr>";
			}
			acum_jsp+="</table>";
			System.out.println(acum_jsp);
			}catch(Exception e){
